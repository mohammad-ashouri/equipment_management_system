...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">فیلم های کوتاه</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'short_video'" :route="'ShortVideos'"></x-search-box>
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد فیلم های کوتاه')
                    <a type="button" href="{{route('ShortVideos.create')}}"
                ...
                @endcan
                @if(empty($shortVideos) or $shortVideos->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($shortVideos as $shortVideo)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($shortVideo->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $shortVideo->adderInfo->name }} {{ $shortVideo->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($shortVideo->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($shortVideo->editorInfo->name))
                                        {{ $shortVideo->editorInfo->name }} {{ $shortVideo->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="flex align-center px-6 py-4">
                                    @can('ویرایش فیلم های کوتاه')
                                        <a href="{{ route('ShortVideos.edit',$shortVideo->id) }}">
                                            <button type="button" data-id="{{ $shortVideo->id }}"
                                    ...
                                    @endcan
                                    @can('حذف فیلم های کوتاه')
                                        {{ html()->form('DELETE')->route('ShortVideos.destroy',$shortVideo->id)->class('delete-post')->open() }}
                                        <button type="submit"
