...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">جزئیات و ویرایش صوت</h1>
            @include('layouts.components.errors')
            <div class="bg-gray-300 rounded shadow flex flex-col ">
                {{ html()->form('PATCH')->route('Audios.update',$audio->id)->acceptsFiles()->id('edit-post')->open() }}
                <div class="bg-white rounded shadow flex flex-col p-4">
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ $audio->title }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="country" name="country" value="{{ $audio->country }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <select name="subject" id="subject"
                                ...
                                @foreach($subjects as $subject)
                                    <option value="{{$subject->id}}" {{$audio->subject==$subject->id ? 'selected' : ''}}>
                                        {{$subject->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="status"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="1" {{$audio->status==1 ? 'selected' : ''}}>منتشر شده</option>
                                <option value="2" {{$audio->status==2 ? 'selected' : ''}}>پیش نویس</option>
                            </select>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            <label for="audio_file"
                                   class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">برای تغییر فایل
                                فعلی انتخاب کنید
                            </label>
                        ...
                        <div>
                            <label for="audio_file"
                                   class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">فایل فعلی
                            </label>
                            <a href="{{ env('APP_URL').$audio->src }}">
                                <button type="button"
