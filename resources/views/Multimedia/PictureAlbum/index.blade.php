...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">آلبوم تصاویر</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'picture_album'" :route="'PictureAlbum'"></x-search-box>
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد آلبوم تصاویر')
                    <a type="button" href="{{route('PictureAlbum.create')}}"
                ...
                @endcan
                @if(empty($pictureAlbums) or $pictureAlbums->isEmpty())
                ...
                @else
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">تاریخ</th>
                        ...
                        @foreach ($pictureAlbums as $pictureAlbum)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($pictureAlbum->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $pictureAlbum->adderInfo->name }} {{ $pictureAlbum->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($pictureAlbum->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($pictureAlbum->editorInfo->name))
                                        {{ $pictureAlbum->editorInfo->name }} {{ $pictureAlbum->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @switch($pictureAlbum->status)
                                        @case(1)
                                            منتشر شده
                                            @break
                                        @case(2)
                                ...
                                <td class="flex align-center px-6 py-4">
                                    @can('ویرایش آلبوم تصاویر')
                                        <a href="{{ route('PictureAlbum.edit',$pictureAlbum->id) }}">
                                            <button type="button" data-id="{{ $pictureAlbum->id }}"
                                    ...
                                    @endcan
                                    @can('حذف آلبوم تصاویر')
                                        {{ html()->form('DELETE')->route('PictureAlbum.destroy',$pictureAlbum->id)->class('delete-post')->open() }}
                                        <button type="submit"
