...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات مایکروفر</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد مایکروفر')
                    <a type="button" href="{{route('Microwaves.create')}}"
                ...
                @endcan
                @if(empty($microwaves) or $microwaves->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($eaters as $microwave)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $microwave->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $microwave->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $microwave->adderInfo->name }} {{ $microwave->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($microwave->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($microwave->editorInfo!=null)
                                        {{ $microwave->editorInfo->name }} {{ $microwave->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($microwave->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش مایکروفر')
                                        <a href="{{ route('Microwaves.edit',$microwave->id) }}">
                                            <button type="button" data-id="{{ $microwave->id }}"
