...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات تخته اعلانات</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد تخته اعلانات')
                    <a type="button" href="{{route('Noticeboards.create')}}"
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($whiteboards as $whiteboard)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    @can('ویرایش تخته اعلانات')
                                        <a href="{{ route('Noticeboards.edit',$whiteboard->id) }}">
