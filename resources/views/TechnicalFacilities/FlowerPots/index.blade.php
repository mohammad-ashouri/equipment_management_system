...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات استند گلدان</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد استند گلدان')
                    <a type="button" href="{{route('FlowerPots.create')}}"
                ...
                @endcan
                @if(empty($flowerPots) or $flowerPots->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($flowerPots as $flowerPot)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $flowerPot->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $flowerPot->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $flowerPot->adderInfo->name }} {{ $flowerPot->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($flowerPot->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($flowerPot->editorInfo!=null)
                                        {{ $flowerPot->editorInfo->name }} {{ $flowerPot->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($flowerPot->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش استند گلدان')
                                        <a href="{{ route('FlowerPots.edit',$flowerPot->id) }}">
                                            <button type="button" data-id="{{ $flowerPot->id }}"
