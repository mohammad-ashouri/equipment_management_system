...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات کپسول آتش نشانی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کپسول آتش نشانی')
                    <a type="button" href="{{route('FireExtinguishers.create')}}"
                       class="px-4 py-2 bg-green-500 w-56 mb-2 text-center text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring focus:border-blue-300">
                        کپسول آتش نشانی جدید
                    </a>
                @endcan
                @if(empty($fireExtinguishers) or $fireExtinguishers->isEmpty())
                ...
                @else
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">وزن</th>
                        ...
                        @foreach ($fireExtinguishers as $fireExtinguisher)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $fireExtinguisher->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    @switch($fireExtinguisher->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $fireExtinguisher->adderInfo->name }} {{ $fireExtinguisher->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($fireExtinguisher->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($fireExtinguisher->editorInfo!=null)
                                        {{ $fireExtinguisher->editorInfo->name }} {{ $fireExtinguisher->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($fireExtinguisher->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کپسول آتش نشانی')
                                        <a href="{{ route('FireExtinguishers.edit',$fireExtinguisher->id) }}">
                                            <button type="button" data-id="{{ $fireExtinguisher->id }}"
