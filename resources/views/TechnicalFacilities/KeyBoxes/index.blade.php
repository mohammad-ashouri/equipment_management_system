...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات جعبه کلید</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد جعبه کلید')
                    <a type="button" href="{{route('KeyBoxes.create')}}"
                ...
                @endcan
                @if(empty($keyBoxes) or $keyBoxes->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">تعداد آویز کلید</th>
                        ...
                        @foreach ($keyBoxes as $keyBox)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $keyBox->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $keyBox->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $keyBox->adderInfo->name }} {{ $keyBox->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($keyBox->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($keyBox->editorInfo!=null)
                                        {{ $keyBox->editorInfo->name }} {{ $keyBox->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($keyBox->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش جعبه کلید')
                                        <a href="{{ route('KeyBoxes.edit',$keyBox->id) }}">
                                            <button type="button" data-id="{{ $keyBox->id }}"
