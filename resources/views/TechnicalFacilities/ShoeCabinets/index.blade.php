...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات جاکفشی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد جاکفشی')
                    <a type="button" href="{{route('ShoeCabinets.create')}}"
                ...
                @endcan
                @if(empty($shoeCabinets) or $shoeCabinets->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($shoeCabinets as $shoeCabinet)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $shoeCabinet->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $shoeCabinet->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $shoeCabinet->adderInfo->name }} {{ $shoeCabinet->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($shoeCabinet->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($shoeCabinet->editorInfo!=null)
                                        {{ $shoeCabinet->editorInfo->name }} {{ $shoeCabinet->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($shoeCabinet->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش جاکفشی')
                                        <a href="{{ route('ShoeCabinets.edit',$shoeCabinet->id) }}">
                                            <button type="button" data-id="{{ $shoeCabinet->id }}"
