...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات تخت خواب</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد تخت خواب')
                    <a type="button" href="{{route('Beds.create')}}"
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($beds as $bed)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $bed->adderInfo->name }} {{ $bed->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($bed->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($bed->editorInfo!=null)
                                        {{ $bed->editorInfo->name }} {{ $bed->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($bed->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش تخت خواب')
                                        <a href="{{ route('Beds.edit',$bed->id) }}">
                                            <button type="button" data-id="{{ $bed->id }}"
