...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات براکت</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد براکت')
                    <a type="button" href="{{route('Brackets.create')}}"
                ...
                @endcan
                @if(empty($brackets) or $brackets->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">مناسب برای</th>
                        ...
                        @foreach ($brackets as $bracket)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $bracket->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $bracket->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $bracket->suitable_for }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    {{ $bracket->adderInfo->name }} {{ $bracket->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($bracket->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($bracket->editorInfo!=null)
                                        {{ $bracket->editorInfo->name }} {{ $bracket->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($bracket->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش براکت')
                                        <a href="{{ route('Brackets.edit',$bracket->id) }}">
                                            <button type="button" data-id="{{ $bracket->id }}"
