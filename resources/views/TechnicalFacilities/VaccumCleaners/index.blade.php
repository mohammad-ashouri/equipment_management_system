...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات جاروبرقی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد جاروبرقی')
                    <a type="button" href="{{route('VaccumCleaners.create')}}"
                ...
                @endcan
                @if(empty($vaccumCleaners) or $vaccumCleaners->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($vaccumCleaners as $vaccumCleaner)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $vaccumCleaner->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $vaccumCleaner->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $vaccumCleaner->adderInfo->name }} {{ $vaccumCleaner->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($vaccumCleaner->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($vaccumCleaner->editorInfo!=null)
                                        {{ $vaccumCleaner->editorInfo->name }} {{ $vaccumCleaner->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($vaccumCleaner->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش جاروبرقی')
                                        <a href="{{ route('VaccumCleaners.edit',$vaccumCleaner->id) }}">
                                            <button type="button" data-id="{{ $vaccumCleaner->id }}"
