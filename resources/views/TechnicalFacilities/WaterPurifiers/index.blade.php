...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات تصفیه کننده آب</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد تصفیه کننده آب')
                    <a type="button" href="{{route('WaterPurifiers.create')}}"
                ...
                @endcan
                @if(empty($waterPurifiers) or $waterPurifiers->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($waterPurifiers as $waterPurifier)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $waterPurifier->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $waterPurifier->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $waterPurifier->adderInfo->name }} {{ $waterPurifier->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($waterPurifier->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($waterPurifier->editorInfo!=null)
                                        {{ $waterPurifier->editorInfo->name }} {{ $waterPurifier->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($waterPurifier->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش تصفیه کننده آب')
                                        <a href="{{ route('WaterPurifiers.edit',$waterPurifier->id) }}">
                                            <button type="button" data-id="{{ $waterPurifier->id }}"
