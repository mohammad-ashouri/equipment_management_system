...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات آبسردکن</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد آبسردکن')
                    <a type="button" href="{{route('WaterDispensers.create')}}"
                ...
                @endcan
                @if(empty($waterDispensers) or $waterDispensers->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">یخچال</th>
                            <th class="px-6 py-3  font-bold ">مخزن</th>
                            <th class="px-6 py-3  font-bold ">تعداد شیر آب سرد</th>
                            <th class="px-6 py-3  font-bold ">تعداد شیر آب گرم</th>
                        ...
                        @foreach ($waterDispensers as $waterDispenser)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $waterDispenser->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    {{ $waterDispenser->adderInfo->name }} {{ $waterDispenser->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($waterDispenser->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($waterDispenser->editorInfo!=null)
                                        {{ $waterDispenser->editorInfo->name }} {{ $waterDispenser->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($waterDispenser->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش آبسردکن')
                                        <a href="{{ route('WaterDispensers.edit',$waterDispenser->id) }}">
                                            <button type="button" data-id="{{ $waterDispenser->id }}"
