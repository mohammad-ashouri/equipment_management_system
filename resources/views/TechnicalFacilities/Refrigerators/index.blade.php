...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات یخچال</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد یخچال')
                    <a type="button" href="{{route('Refrigerators.create')}}"
                ...
                @endcan
                @if(empty($refrigerators) or $refrigerators->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($refrigerators as $refrigerator)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $refrigerator->adderInfo->name }} {{ $refrigerator->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($refrigerator->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($refrigerator->editorInfo!=null)
                                        {{ $refrigerator->editorInfo->name }} {{ $refrigerator->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($refrigerator->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش یخچال')
                                        <a href="{{ route('Refrigerators.edit',$refrigerator->id) }}">
                                            <button type="button" data-id="{{ $refrigerator->id }}"
