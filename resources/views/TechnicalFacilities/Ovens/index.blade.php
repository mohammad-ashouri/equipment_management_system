...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات اجاق</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد اجاق')
                    <a type="button" href="{{route('Ovens.create')}}"
                ...
                @endcan
                @if(empty($ovens) or $ovens->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($ovens as $oven)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $oven->adderInfo->name }} {{ $oven->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($oven->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($oven->editorInfo!=null)
                                        {{ $oven->editorInfo->name }} {{ $oven->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($oven->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش اجاق')
                                        <a href="{{ route('Ovens.edit',$oven->id) }}">
                                            <button type="button" data-id="{{ $oven->id }}"
