...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات کولر آبی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کولر آبی')
                    <a type="button" href="{{route('IranianCoolers.create')}}"
                ...
                @endcan
                @if(empty($iranianCoolers) or $iranianCoolers->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">حجم کولر</th>
                        ...
                        @foreach ($iranianCoolers as $iranianCooler)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $iranianCooler->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $iranianCooler->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $iranianCooler->capacity }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $iranianCooler->type }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($iranianCooler->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $iranianCooler->adderInfo->name }} {{ $iranianCooler->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($iranianCooler->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($iranianCooler->editorInfo!=null)
                                        {{ $iranianCooler->editorInfo->name }} {{ $iranianCooler->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($iranianCooler->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کولر آبی')
                                        <a href="{{ route('IranianCoolers.edit',$iranianCooler->id) }}">
                                            <button type="button" data-id="{{ $iranianCooler->id }}"
