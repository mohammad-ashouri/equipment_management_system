...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات کولر گازی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کولر گازی')
                    <a type="button" href="{{route('AirConditioners.create')}}"
                ...
                @endcan
                @if(empty($airConditioners) or $airConditioners->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($airConditioners as $airConditioner)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $airConditioner->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $airConditioner->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $airConditioner->power_type }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($airConditioner->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $airConditioner->adderInfo->name }} {{ $airConditioner->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($airConditioner->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($airConditioner->editorInfo!=null)
                                        {{ $airConditioner->editorInfo->name }} {{ $airConditioner->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($airConditioner->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کولر گازی')
                                        <a href="{{ route('AirConditioners.edit',$airConditioner->id) }}">
                                            <button type="button" data-id="{{ $airConditioner->id }}"
