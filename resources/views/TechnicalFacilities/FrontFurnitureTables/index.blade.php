...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات جلومبلی/میز عسلی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد جلومبلی/میز عسلی')
                    <a type="button" href="{{route('FrontFurnitureTables.create')}}"
                       class="px-4 py-2 bg-green-500 w-40 mb-2 text-center text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring focus:border-blue-300">
                        جلومبلی/میز عسلی جدید
                    </a>
                @endcan
                @if(empty($frontFurnitureTables) or $frontFurnitureTables->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($frontFurnitureTables as $frontFurnitureTable)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $frontFurnitureTable->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $frontFurnitureTable->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $frontFurnitureTable->material }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $frontFurnitureTable->width }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    {{ $frontFurnitureTable->adderInfo->name }} {{ $frontFurnitureTable->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($frontFurnitureTable->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($frontFurnitureTable->editorInfo!=null)
                                        {{ $frontFurnitureTable->editorInfo->name }} {{ $frontFurnitureTable->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($frontFurnitureTable->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش جلومبلی/میز عسلی')
                                        <a href="{{ route('FrontFurnitureTables.edit',$frontFurnitureTable->id) }}">
                                            <button type="button" data-id="{{ $frontFurnitureTable->id }}"
