...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات نردبان</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد نردبان')
                    <a type="button" href="{{route('Ladders.create')}}"
                ...
                @endcan
                @if(empty($ladders) or $ladders->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">تعداد پله</th>
                        ...
                        @foreach ($ladders as $ladder)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $ladder->adderInfo->name }} {{ $ladder->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($ladder->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($ladder->editorInfo!=null)
                                        {{ $ladder->editorInfo->name }} {{ $ladder->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($ladder->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش نردبان')
                                        <a href="{{ route('Ladders.edit',$ladder->id) }}">
                                            <button type="button" data-id="{{ $ladder->id }}"
