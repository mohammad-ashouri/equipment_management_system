...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات چای ساز</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد چای ساز')
                    <a type="button" href="{{route('TeaMakers.create')}}"
                ...
                @endcan
                @if(empty($teaMakers) or $teaMakers->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($teaMakers as $teaMaker)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $teaMaker->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $teaMaker->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($teaMaker->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $teaMaker->adderInfo->name }} {{ $teaMaker->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($teaMaker->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($teaMaker->editorInfo!=null)
                                        {{ $teaMaker->editorInfo->name }} {{ $teaMaker->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($teaMaker->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش چای ساز')
                                        <a href="{{ route('TeaMakers.edit',$teaMaker->id) }}">
                                            <button type="button" data-id="{{ $teaMaker->id }}"
