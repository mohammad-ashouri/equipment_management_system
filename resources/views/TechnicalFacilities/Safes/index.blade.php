...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات گاو صندوق</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد گاو صندوق')
                    <a type="button" href="{{route('Safes.create')}}"
                ...
                @endcan
                @if(empty($safes) or $safes->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($safes as $safe)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $safe->adderInfo->name }} {{ $safe->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($safe->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($safe->editorInfo!=null)
                                        {{ $safe->editorInfo->name }} {{ $safe->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($safe->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش گاو صندوق')
                                        <a href="{{ route('Safes.edit',$safe->id) }}">
                                            <button type="button" data-id="{{ $safe->id }}"
