...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات برش دهنده کاغذ</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد برش دهنده کاغذ')
                    <a type="button" href="{{route('PaperCutters.create')}}"
                ...
                @endcan
                @if(empty($paperCutters) or $paperCutters->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($paperCutters as $paperCutter)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $paperCutter->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $paperCutter->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $paperCutter->adderInfo->name }} {{ $paperCutter->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($paperCutter->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($paperCutter->editorInfo!=null)
                                        {{ $paperCutter->editorInfo->name }} {{ $paperCutter->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($paperCutter->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش برش دهنده کاغذ')
                                        <a href="{{ route('PaperCutters.edit',$paperCutter->id) }}">
                                            <button type="button" data-id="{{ $paperCutter->id }}"
