...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات پشتیبانی - مدیریت بر اطلاعات مبلمان</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد مبلمان')
                    <a type="button" href="{{route('Couches.create')}}"
                ...
                @endcan
                @if(empty($couches) or $couches->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($couches as $couch)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $couch->adderInfo->name }} {{ $couch->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($couch->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($couch->editorInfo!=null)
                                        {{ $couch->editorInfo->name }} {{ $couch->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($couch->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش مبلمان')
                                        <a href="{{ route('Couches.edit',$couch->id) }}">
                                            <button type="button" data-id="{{ $couch->id }}"
