...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">پرونده ویژه</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'special_case'" :route="'SpecialCases'"></x-search-box>
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد پرونده ویژه')
                    <a type="button" href="{{route('SpecialCases.create')}}"
                ...
                @endcan
                @if(empty($specialCases) or $specialCases->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($specialCases as $specialCase)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($specialCase->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $specialCase->adderInfo->name }} {{ $specialCase->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($specialCase->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($specialCase->editorInfo->name))
                                        {{ $specialCase->editorInfo->name }} {{ $specialCase->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="flex align-center px-6 py-4">
                                    @can('ویرایش پرونده ویژه')
                                        <a href="{{ route('SpecialCases.edit',$specialCase->id) }}">
                                            <button type="button" data-id="{{ $specialCase->id }}"
                                    ...
                                    @endcan
                                    @can('حذف پرونده ویژه')
                                        {{ html()->form('DELETE')->route('SpecialCases.destroy',$specialCase->id)->class('delete-post')->open() }}
                                        <button type="submit"
