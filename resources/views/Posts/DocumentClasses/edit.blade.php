...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <div class="flex mb-4">
                <h1 class="text-2xl font-bold mb-4">ویرایش و جزئیات کلاس اسناد</h1>
                @if ($documentClass->status==2)
                    <x-show-draft-button route="DocumentClasses"
                                         token="{{ $documentClass->draft_token }}"></x-show-draft-button>
                @endif
            </div>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-gray-300 rounded shadow flex flex-col ">
                {{ html()->form('PATCH')->route('DocumentClasses.update',$documentClass->id)->acceptsFiles()->id('edit-post')->open() }}
                <div class="bg-white rounded shadow flex flex-col p-4">
                    <div class="grid gap-6 mb-6 md:grid-cols-3">
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ $documentClass->title }}"
                        ...
                        <div>
                            ...
                            <textarea id="short_title" name="short_title" rows="7"
                                      class="border rounded-md w-full px-3 py-2 focus:outline-none focus:ring focus:border-blue-300">{{ $documentClass->short_title }}</textarea>
                        </div>
                        <div class="flex grid-cols-2">
                            @php
                                $time= list($hours, $minutes) = explode(':', $documentClass->class_time);
                            @endphp
                            <div class="ml-2">
                                <label for=""
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="1" {{$documentClass->status==1 ? 'selected' : ''}}>منتشر شده</option>
                                <option value="2" {{$documentClass->status==2 ? 'selected' : ''}}>پیش نویس</option>
                            </select>
                        </div>
                        <div>
                            <label for="chosen"
                            ...
                            <select name="chosen"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="0" {{$documentClass->chosen==0 ? 'selected' : ''}}>خیر</option>
                                <option value="1" {{$documentClass->chosen==1 ? 'selected' : ''}}>بله</option>
                            </select>
                        </div>
                    </div>
                    <div>
                        ...
                        <textarea id="body" name="body" rows="7"
                                  class="border rounded-md w-full px-3 py-2 focus:outline-none focus:ring focus:border-blue-300">{{ $documentClass->about_class }}</textarea>
                    </div>
                </div>
                <div class="bg-white mt-10 rounded shadow flex flex-col p-4">
                    ...
                    <div class="grid gap-6 mb-6" id="topicsContainer">
                        <div class="grid-cols-2">
                            @foreach(json_decode($documentClass->topics,true) as $index => $topic)
                                <div class="flex">
                                    ...
                                    <div class="w-full mr-2">
                                        ...
                                        <input type="text" id="aparat{{ $index }}" name="aparats[]"
                                               value="{{ $topic['link'] }}"
                                               class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <div>
                            <img class="w-full h-96 cursor-pointer" title="برای بزرگنمایی کلیک کنید"
                                 @if($documentClass->mainImage!=null) onclick="openModal('{{ env('APP_URL') . $documentClass->mainImage->src }}')"
                                 @endif
                                 src="{{$documentClass->mainImage!=null ? env('APP_URL').$documentClass->mainImage->src : 'not found!'}}"
                                 alt="تصویر یافت نشد!">
                        </div>
                    </div>
                </div>
                ...
                {{--                        <div>--}}
                {{--                            <label for="video"--}}
                {{--                                   class="block mb-2 text-sm font-medium text-gray-900 dark:text-white"> جهت تغییر فیلم--}}
                {{--                                دوره انتخاب کنید--}}
                {{--                            </label>--}}
                ...
                {{--                                <source src="{{$documentClass->mainVideo->src}}" type="video/mp4">--}}
                ...
                <div class="bg-white mt-10 rounded shadow flex flex-col p-4">
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            ...
                            <select name="teacher" id="teacher"
                                ...
                                @foreach($teachers as $teacher)
                                    <option
                                        value="{{ $teacher->id }}" {{$documentClass->teacher==$teacher->id ? 'selected' : ''}}>
                        ...
                        <div>
                            <img class="w-full h-96 cursor-pointer" title="برای بزرگنمایی کلیک کنید"
                                 src="{{$teacher->mainImage->src}}" id="master_image"
                                 alt="تصویر یافت نشد!">
                        </div>
                    </div>
                </div>
                ...
                {{--                Slider manager--}}
                <x-slider.update :image="$documentClass->sliderImage"/>

                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    @can('ویرایش کلاس اسناد')
                        <button type="submit"
