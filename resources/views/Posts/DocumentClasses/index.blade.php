...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">کلاس اسناد</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'document_class'" :route="'DocumentClasses'"></x-search-box>
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کلاس اسناد')
                    <a type="button" href="{{route('DocumentClasses.create')}}"
                ...
                @endcan
                @if(empty($documentClasses) or $documentClasses->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($documentClasses as $documentClass)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($documentClass->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $documentClass->adderInfo->name }} {{ $documentClass->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($documentClass->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($documentClass->editorInfo->name))
                                        {{ $documentClass->editorInfo->name }} {{ $documentClass->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @switch($documentClass->status)
                                        @case(1)
                                            منتشر شده
                                            @break
                                        @case(2)
                                ...
                                <td class="flex align-center px-6 py-4">
                                    @can('ویرایش کلاس اسناد')
                                        <a href="{{ route('DocumentClasses.edit',$documentClass->id) }}">
                                            <button type="button" data-id="{{ $documentClass->id }}"
                                    ...
                                    @endcan
                                    @can('حذف کلاس اسناد')
                                        {{ html()->form('DELETE')->route('DocumentClasses.destroy',$documentClass->id)->class('delete-post')->open() }}
                                        <button type="submit"
