...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <div class="flex mb-4">
                <h1 class="text-2xl font-bold mb-4">جزئیات و ویرایش فضای مجازی</h1>
                @if ($socialMedia->status==2)
                    <x-show-draft-button route="SocialMedias"
                                         token="{{ $socialMedia->draft_token }}"></x-show-draft-button>
                @endif
            </div>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-gray-300 rounded shadow flex flex-col ">
                {{ html()->form('PATCH')->route('SocialMedia.update',$socialMedia->id)->acceptsFiles()->id('edit-post')->open() }}
                <div class="bg-white rounded shadow flex flex-col p-4">
                    <div class="grid gap-6 mb-6 md:grid-cols-3">
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ $socialMedia->title }}"
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="1" {{$socialMedia->status==1 ? 'selected' : ''}}>منتشر شده</option>
                                <option value="2" {{$socialMedia->status==2 ? 'selected' : ''}}>پیش نویس</option>
                            </select>
                        </div>
                        <div>
                            <label for="chosen"
                            ...
                            <select name="chosen"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="0" {{$socialMedia->chosen==0 ? 'selected' : ''}}>خیر</option>
                                <option value="1" {{$socialMedia->chosen==1 ? 'selected' : ''}}>بله</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="7"
                                  class="border rounded-md w-full px-3 py-2 focus:outline-none focus:ring focus:border-blue-300">{{ $socialMedia->body }}</textarea>
                    </div>
                </div>
                <div class="bg-white mt-10 rounded shadow flex flex-col p-4">
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            >
                                ...
                                @foreach($platforms as $platform)
                                    <option {{ $socialMedia->platform ? 'selected' : '' }} value="{{$platform->id}}">
                                        {{$platform->name}}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label for="followers"
                                   class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">تعداد دنبال
                                کنندگان
                            </label>
                        ...
                        <div>
                            ...
                            <input type="text" id="activity" name="activity" value="{{ $socialMedia->activity }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <input type="text" id="keywords" name="keywords"
                                   value="@if($socialMedia->keywords != null){{ implode(', ', array_map('trim', json_decode($socialMedia->keywords, true))) }}@endif"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <div>
                            <img class="w-full h-96  cursor-pointer" title="برای بزرگنمایی کلیک کنید"
                                 @if($socialMedia->mainImage!=null) onclick="openModal('{{ env('APP_URL') . $socialMedia->mainImage->src }}')"
                                 @endif
                                 src="{{$socialMedia->mainImage!=null ? env('APP_URL').$socialMedia->mainImage->src : 'not found!'}}"
                                 alt="تصویر یافت نشد!">
                        </div>
                    </div>
                </div>
                <div class="bg-white mt-10 rounded shadow flex flex-col p-4">
                    ...
                    @if($socialMedia->childImages!=null)
                        <div class="grid grid-cols-4 gap-3">
                            @foreach($socialMedia->childImages as $images)
                                <div class="relative inline-block image-container">
                                    <img class="w-96 h-96 rounded border border-black other-image"
                                         data-id="{{$images->id}}"
                ...
                {{--                Slider manager--}}
                <x-slider.update :image="$socialMedia->sliderImage"/>
