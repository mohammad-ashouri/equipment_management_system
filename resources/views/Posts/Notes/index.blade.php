...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">یادداشت ها</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'note'" :route="'Notes'"></x-search-box>
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد یادداشت ها')
                    <a type="button" href="{{route('Notes.create')}}"
                ...
                @endcan
                @if(empty($notes) or $notes->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($notes as $note)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($note->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $note->adderInfo->name }} {{ $note->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($note->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($note->editorInfo->name))
                                        {{ $note->editorInfo->name }} {{ $note->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @switch($note->status)
                                ...
                                <td class="flex align-center px-6 py-4">
                                    ...
                                    @endcan
                                    @can('حذف یادداشت ها')
                                        {{ html()->form('DELETE')->route('Notes.destroy',$note->id)->class('delete-post')->open() }}
                                        <button type="submit"
