...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">اسناد</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'post'" :route="'Posts'"></x-search-box>
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($posts as $post)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($post->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $post->adderInfo->name }} {{ $post->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($post->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($post->editorInfo->name))
                                        {{ $post->editorInfo->name }} {{ $post->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @switch($post->status)
                                ...
                                <td class="flex align-center px-6 py-4">
                                    ...
                                    @endcan
                                    @can('حذف اسناد')
                                        {{ html()->form('DELETE')->route('Posts.destroy',$post->id)->class('delete-post')->open() }}
                                        <button type="submit"
