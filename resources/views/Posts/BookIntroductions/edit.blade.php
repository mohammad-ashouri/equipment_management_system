...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <div class="flex mb-4">
            <h1 class="text-2xl font-bold mb-4">جزئیات و ویرایش معرفی کتاب</h1>
            </div>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-gray-300 rounded shadow flex flex-col ">
                {{ html()->form('PATCH')->route('BookIntroductions.update',$bookIntroduction->id)->acceptsFiles()->id('edit-post')->open() }}
                <div class="bg-white rounded shadow flex flex-col p-4">
                    <div class="grid gap-6 mb-6 md:grid-cols-3">
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ $bookIntroduction->title }}"
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="1" {{$bookIntroduction->status==1 ? 'selected' : ''}}>منتشر شده</option>
                                <option value="2" {{$bookIntroduction->status==2 ? 'selected' : ''}}>پیش نویس</option>
                            </select>
                        </div>
                        <div>
                            <label for="chosen"
                            ...
                            <select name="chosen"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="0" {{$bookIntroduction->chosen==0 ? 'selected' : ''}}>خیر</option>
                                <option value="1" {{$bookIntroduction->chosen==1 ? 'selected' : ''}}>بله</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="7"
                                  class="border rounded-md w-full px-3 py-2 focus:outline-none focus:ring focus:border-blue-300">{{ $bookIntroduction->body }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" id="book_title" name="book_title"
                                   value="{{ $bookIntroduction->book_title }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   placeholder="عنوان را وارد کنید" required>
                        </div>
                        <div>
                            ...
                            <input type="text" id="author" name="author" value="{{ $bookIntroduction->author }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        ...
                        <div>
                            ...
                            <input type="text" id="publisher" name="publisher"
                                   value="{{ $bookIntroduction->publisher }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   placeholder="نویسنده/مترجم را وارد کنید" required>
                        </div>
                        <div>
                            ...
                            <input type="text" id="subjects" name="subjects" value="{{ $bookIntroduction->subjects }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   placeholder="نویسنده/مترجم را وارد کنید" required>
                        </div>
                        <div>
                            ...
                            <textarea id="hint" name="hint" rows="7" placeholder="اشاره را وارد کنید"
                                      class="border rounded-md w-full px-3 py-2 focus:outline-none focus:ring focus:border-blue-300">{{ $bookIntroduction->hint }}</textarea>
                        </div>
                        <div>
                            <label for="keywords"
                            ...
                            <input type="text" id="keywords" name="keywords"
                                   value="@if($bookIntroduction->keywords != null){{ implode(', ', array_map('trim', json_decode($bookIntroduction->keywords, true))) }}@endif"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <div>
                            <img class="w-full h-96 cursor-pointer" title="برای بزرگنمایی کلیک کنید"
                                 @if($bookIntroduction->mainImage!=null) onclick="openModal('{{ env('APP_URL') . $bookIntroduction->mainImage->src }}')"
                                 @endif
                                 src="{{$bookIntroduction->mainImage!=null ? env('APP_URL').$bookIntroduction->mainImage->src : 'not found!'}}"
                                 alt="تصویر یافت نشد!">
                        </div>
                    </div>
                </div>
                <div class="bg-white mt-10 rounded shadow flex flex-col p-4">
                    ...
                    @if($bookIntroduction->childImages!=null)
                        <div class="grid grid-cols-4 gap-3">
                            @foreach($bookIntroduction->childImages as $images)
                ...
                <x-related-items-update :postTypes="$postTypes" :relatedItems="$bookIntroduction->related_items"/>

                {{--                Similar book introdcutions update--}}
                <x-similar-book-introductions-update :bookIntroductions="$bookIntroductions"/>

                {{--                Slider manager--}}
                <x-slider.update :image="$bookIntroduction->sliderImage"/>

                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    @can('ویرایش معرفی کتاب')
                        <input type="hidden" name="id" value="{{ $bookIntroduction->id }}">
                        <button type="submit"
