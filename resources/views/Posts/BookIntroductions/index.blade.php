...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">معرفی کتاب</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'book_introduction'" :route="'BookIntroductions'"></x-search-box>
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد معرفی کتاب')
                    <a type="button" href="{{route('BookIntroductions.create')}}"
                ...
                @endcan
                @if(empty($bookIntroductions) or $bookIntroductions->isEmpty())
                ...
                @else
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($bookIntroductions as $bookIntroduction)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($bookIntroduction->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $bookIntroduction->adderInfo->name }} {{ $bookIntroduction->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($bookIntroduction->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($bookIntroduction->editorInfo->name))
                                        {{ $bookIntroduction->editorInfo->name }} {{ $bookIntroduction->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @switch($bookIntroduction->status)
                                        @case(1)
                                            منتشر شده
                                            @break
                                        @case(2)
                                ...
                                <td class="flex align-center px-6 py-4">
                                    @can('ویرایش معرفی کتاب')
                                        <a href="{{ route('BookIntroductions.edit',$bookIntroduction->id) }}">
                                            <button type="button" data-id="{{ $bookIntroduction->id }}"
                                    ...
                                    @endcan
                                    @can('حذف معرفی کتاب')
                                        {{ html()->form('DELETE')->route('BookIntroductions.destroy',$bookIntroduction->id)->class('delete-post')->open() }}
                                        <button type="submit"
