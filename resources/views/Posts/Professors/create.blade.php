@php use App\Models\Professor; @endphp
...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <div class="flex mb-4">
            <h1 class="text-2xl font-bold mb-4">ایجاد معرفی کتاب</h1>
                @if (isset($id) and $model->status==2)
                    <x-show-draft-button route="Professors" token="{{ $model->draft_token }}"></x-show-draft-button>
                @endif
            </div>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-gray-300 rounded shadow flex flex-col ">
                {{ html()->form('POST')->route('Professors.store')->acceptsFiles()->id('create-post')->open() }}
                        ...
                        <div>
                            ...
                            >
                                ...
                                @foreach($adjectives as $adjective)
                                    <option {{ old('adjective') ? 'selected' : '' }} value="{{$adjective->id}}">
                                        {{$adjective->name}}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="bg-white mt-10 rounded shadow flex flex-col p-4">
                    ...
                    <div class="grid gap-2 mb-6 grid-cols-2" id="booksContainer">
                            @if(old('books'))
                                @foreach(old('books') as $index => $book)
                                    @if($book==null)
                                        @continue
                                    @endif
                                    <div class="mt-2 book-row">
                                        <div class="w-full">
                                            <label for=""
