...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سوژه های رسانه ای</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <x-search-box :postType="'media_subject'" :route="'MediaSubjects'"></x-search-box>
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد سوژه های رسانه ای')
                    <a type="button" href="{{route('MediaSubjects.create')}}"
                ...
                @endcan
                @if(empty($mediaSubjects) or $mediaSubjects->isEmpty())
                ...
                @else
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($mediaSubjects as $mediaSubject)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($mediaSubject->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $mediaSubject->adderInfo->name }} {{ $mediaSubject->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($mediaSubject->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if(!empty($mediaSubject->editorInfo->name))
                                        {{ $mediaSubject->editorInfo->name }} {{ $mediaSubject->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @switch($mediaSubject->status)
                                        @case(1)
                                            منتشر شده
                                            @break
                                        @case(2)
                                ...
                                <td class="flex align-center px-6 py-4">
                                    @can('ویرایش سوژه های رسانه ای')
                                        <a href="{{ route('MediaSubjects.edit',$mediaSubject->id) }}">
                                            <button type="button" data-id="{{ $mediaSubject->id }}"
                                    ...
                                    @endcan
                                    @can('حذف سوژه های رسانه ای')
                                        {{ html()->form('DELETE')->route('MediaSubjects.destroy',$mediaSubject->id)->class('delete-post')->open() }}
                                        <button type="submit"
