...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <div class="flex mb-4">
                <h1 class="text-2xl font-bold mb-4">جزئیات و ویرایش سوژه رسانه ای</h1>
                @if ($mediaSubject->status==2)
                    <x-show-draft-button route="MediaSubjects"
                                         token="{{ $mediaSubject->draft_token }}"></x-show-draft-button>
                @endif
            </div>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-gray-300 rounded shadow flex flex-col ">
                {{ html()->form('PATCH')->route('MediaSubjects.update',$mediaSubject->id)->acceptsFiles()->id('edit-post')->open() }}
                <div class="bg-white rounded shadow flex flex-col p-4">
                    <div class="grid gap-6 mb-6 md:grid-cols-3">
                        <div>
                            ...
                            <input type="text" id="title" name="title" value="{{ $mediaSubject->title }}"
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="1" {{$mediaSubject->status==1 ? 'selected' : ''}}>منتشر شده</option>
                                <option value="2" {{$mediaSubject->status==2 ? 'selected' : ''}}>پیش نویس</option>
                            </select>
                        </div>
                        <div>
                            <label for="chosen"
                            ...
                            <select name="chosen"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="0" {{$mediaSubject->chosen==0 ? 'selected' : ''}}>خیر</option>
                                <option value="1" {{$mediaSubject->chosen==1 ? 'selected' : ''}}>بله</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="subject_format"
                                ...
                                @foreach($subjectFormats as $subjectFormat)
                                    <option
                                        value="{{ $subjectFormat->id }}" {{$mediaSubject->subject_format==$subjectFormat->id ? 'selected' : ''}}>
                                        {{$subjectFormat->name}}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="subject_audience"
                                ...
                                @foreach($subjectAudiences as $subjectAudience)
                                    <option
                                        value="{{ $subjectAudience->id }}" {{$subjectAudience->subject_format==$subjectAudience->id ? 'selected' : ''}}>
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="7"
                                  class="border rounded-md w-full px-3 py-2 focus:outline-none focus:ring focus:border-blue-300">{{ $mediaSubject->body }}</textarea>
                    </div>
                </div>
                    ...
                    <div class="grid gap-6 mb-6 ">
                        <div>
                            ...
                            <textarea id="resources" name="resources" rows="7"
                                      class="border rounded-md w-full px-3 py-2 focus:outline-none focus:ring focus:border-blue-300">{{ $mediaSubject->resources }}</textarea>
                        </div>
                    </div>
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        <div>
                            ...
                            <input type="text" id="keywords" name="keywords"
                                   value="@if($mediaSubject->keywords != null){{ implode(', ', array_map('trim', json_decode($mediaSubject->keywords, true))) }}@endif"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <div>
                            <img class="w-full h-96 cursor-pointer" title="برای بزرگنمایی کلیک کنید"
                                 @if($mediaSubject->mainImage!=null) onclick="openModal('{{ env('APP_URL') . $mediaSubject->mainImage->src }}')"
                                 @endif
                                 src="{{$mediaSubject->mainImage!=null ? env('APP_URL').$mediaSubject->mainImage->src : 'not found!'}}"
                                 alt="تصویر یافت نشد!">
                        </div>
                    </div>
                </div>
                <div class="bg-white mt-10 rounded shadow flex flex-col p-4">
                    ...
                    @if($mediaSubject->childImages!=null)
                        <div class="grid grid-cols-4 gap-3">
                            @foreach($mediaSubject->childImages as $images)
                ...
                <x-attached-documents-update :documents="json_decode($mediaSubject->attached_documents,true)"/>

                {{--                Related items update--}}
                <x-related-items-update :postTypes="$postTypes" :relatedItems="$mediaSubject"/>

                {{--                Similar media subjects update--}}
                <x-similar-media-subjects-update :mediaSubjects="$mediaSubject"/>

                {{--                Slider manager--}}
                <x-slider.update :image="$mediaSubject->sliderImage"/>

                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    @can('ویرایش سوژه های رسانه ای')
                        <input type="hidden" name="id" value="{{ $mediaSubject->id }}">
                        <button type="submit"
