@php use App\Models\ResearchSubject; @endphp
@extends('layouts.PanelMaster')
@php
    if (isset($id)){
        $model=ResearchSubject::find($id);
    }
@endphp
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <div class="flex mb-4">
            <h1 class="text-2xl font-bold mb-4">ایجاد سوژه پژوهشی</h1>
                @if (isset($id) and $model->status==2)
                    <x-show-draft-button route="ResearchSubjects" token="{{ $model->draft_token }}"></x-show-draft-button>
                @endif
            </div>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-gray-300 rounded shadow flex flex-col ">
                {{ html()->form('POST')->route('ResearchSubjects.store')->acceptsFiles()->id('create-post')->open() }}
                ...
                {{--                Similar research subjects create--}}
                <x-similar-research-subjects-create :researchSubjects="old('research_subjects', [])"/>
