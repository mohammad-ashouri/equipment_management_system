...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات سوییچ ماهواره</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد سوییچ ماهواره')
                    <a type="button" href="{{route('SatelliteSwitches.create')}}"
                ...
                @endcan
                @if(empty($satelliteSwitches) or $satelliteSwitches->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($satelliteSwitches as $satelliteSwitch)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $satelliteSwitch->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $satelliteSwitch->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($satelliteSwitch->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $satelliteSwitch->adderInfo->name }} {{ $satelliteSwitch->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($satelliteSwitch->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($satelliteSwitch->editorInfo!=null)
                                        {{ $satelliteSwitch->editorInfo->name }} {{ $satelliteSwitch->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($satelliteSwitch->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش سوییچ ماهواره')
                                        <a href="{{ route('SatelliteSwitches.edit',$satelliteSwitch->id) }}">
                                            <button type="button" data-id="{{ $satelliteSwitch->id }}"
