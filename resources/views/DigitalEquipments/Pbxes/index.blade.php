...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات دستگاه سانترال</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد دستگاه سانترال')
                    <a type="button" href="{{route('Pbxes.create')}}"
                ...
                @endcan
                @if(empty($pbxes) or $pbxes->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($pbxes as $pbx)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $pbx->adderInfo->name }} {{ $pbx->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($pbx->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($pbx->editorInfo!=null)
                                        {{ $pbx->editorInfo->name }} {{ $pbx->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($pbx->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش دستگاه سانترال')
                                        <a href="{{ route('Pbxes.edit',$pbx->id) }}">
                                            <button type="button" data-id="{{ $pbx->id }}"
