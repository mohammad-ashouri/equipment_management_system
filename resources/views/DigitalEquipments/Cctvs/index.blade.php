...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات دوربین مدار بسته</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد دوربین مدار بسته')
                    <a type="button" href="{{route('Cctvs.create')}}"
                       class="px-4 py-2 bg-green-500 w-56 mb-2 text-center text-white rounded-md hover:bg-green-600 focus:outline-none focus:ring focus:border-blue-300">
                        دوربین مدار بسته جدید
                    </a>
                @endcan
                @if(empty($cctvs) or $cctvs->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($cctvs as $cctv)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $cctv->adderInfo->name }} {{ $cctv->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cctv->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($cctv->editorInfo!=null)
                                        {{ $cctv->editorInfo->name }} {{ $cctv->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cctv->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش دوربین مدار بسته')
                                        <a href="{{ route('Cctvs.edit',$cctv->id) }}">
                                            <button type="button" data-id="{{ $cctv->id }}"
