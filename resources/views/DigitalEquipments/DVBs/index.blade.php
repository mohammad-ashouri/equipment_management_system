...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات کارت DVB</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کارت DVB')
                    <a type="button" href="{{route('DVBs.create')}}"
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">تعداد ورودی</th>
                        ...
                        @foreach ($dvbs as $dvb)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $dvb->adderInfo->name }} {{ $dvb->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($dvb->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($dvb->editorInfo!=null)
                                        {{ $dvb->editorInfo->name }} {{ $dvb->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($dvb->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کارت DVB')
                                        <a href="{{ route('DVBs.edit',$dvb->id) }}">
                                            <button type="button" data-id="{{ $dvb->id }}"
