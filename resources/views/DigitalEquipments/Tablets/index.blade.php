...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات تبلت</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد تبلت')
                    <a type="button" href="{{route('Tablets.create')}}"
                ...
                @endcan
                @if(empty($tablets) or $tablets->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($tablets as $tablet)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $tablet->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $tablet->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $tablet->internal_memory }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $tablet->ram }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $tablet->simcard_number }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    {{ $tablet->adderInfo->name }} {{ $tablet->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($tablet->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($tablet->editorInfo!=null)
                                        {{ $tablet->editorInfo->name }} {{ $tablet->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($tablet->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش تبلت')
                                        <a href="{{ route('Tablets.edit',$tablet->id) }}">
                                            <button type="button" data-id="{{ $tablet->id }}"
