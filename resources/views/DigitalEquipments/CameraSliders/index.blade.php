...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات اسلایدر دوربین</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد اسلایدر دوربین')
                    <a type="button" href="{{route('CameraSliders.create')}}"
                ...
                @endcan
                @if(empty($cameraRams) or $cameraRams->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($cameraRams as $cameraRam)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $cameraRam->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $cameraRam->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($cameraRam->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $cameraRam->adderInfo->name }} {{ $cameraRam->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cameraRam->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($cameraRam->editorInfo!=null)
                                        {{ $cameraRam->editorInfo->name }} {{ $cameraRam->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cameraRam->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش اسلایدر دوربین')
                                        <a href="{{ route('CameraSliders.edit',$cameraRam->id) }}">
                                            <button type="button" data-id="{{ $cameraRam->id }}"
