...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات گیرنده دیجیتال</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد گیرنده دیجیتال')
                    <a type="button" href="{{route('SetTopBoxes.create')}}"
                ...
                @endcan
                @if(empty($setTopBoxes) or $setTopBoxes->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($setTopBoxes as $setTopBox)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $setTopBox->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $setTopBox->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($setTopBox->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $setTopBox->adderInfo->name }} {{ $setTopBox->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($setTopBox->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($setTopBox->editorInfo!=null)
                                        {{ $setTopBox->editorInfo->name }} {{ $setTopBox->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($setTopBox->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش گیرنده دیجیتال')
                                        <a href="{{ route('SetTopBoxes.edit',$setTopBox->id) }}">
                                            <button type="button" data-id="{{ $setTopBox->id }}"
