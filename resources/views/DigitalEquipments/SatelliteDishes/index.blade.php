...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات دیش ماهواره</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد دیش ماهواره')
                    <a type="button" href="{{route('SatelliteDishes.create')}}"
                ...
                @endcan
                @if(empty($satelliteDishes) or $satelliteDishes->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($satelliteDishes as $satelliteDish)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $satelliteDish->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $satelliteDish->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($satelliteDish->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $satelliteDish->adderInfo->name }} {{ $satelliteDish->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($satelliteDish->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($satelliteDish->editorInfo!=null)
                                        {{ $satelliteDish->editorInfo->name }} {{ $satelliteDish->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($satelliteDish->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش دیش ماهواره')
                                        <a href="{{ route('SatelliteDishes.edit',$satelliteDish->id) }}">
                                            <button type="button" data-id="{{ $satelliteDish->id }}"
