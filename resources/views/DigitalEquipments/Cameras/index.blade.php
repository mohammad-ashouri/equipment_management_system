...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات دوربین</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد دوربین')
                    <a type="button" href="{{route('Cameras.create')}}"
                ...
                @endcan
                @if(empty($cameras) or $cameras->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($cameras as $camera)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $camera->adderInfo->name }} {{ $camera->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($camera->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($camera->editorInfo!=null)
                                        {{ $camera->editorInfo->name }} {{ $camera->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($camera->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش دوربین')
                                        <a href="{{ route('Cameras.edit',$camera->id) }}">
                                            <button type="button" data-id="{{ $camera->id }}"
