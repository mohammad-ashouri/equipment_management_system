...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات اسپیکر</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد اسپیکر')
                    <a type="button" href="{{route('Speakers.create')}}"
                ...
                @endcan
                @if(empty($speakers) or $speakers->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">تعداد قطعات</th>
                        ...
                        @foreach ($speakers as $speaker)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $speaker->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $speaker->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $speaker->parts_number }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    {{ $speaker->adderInfo->name }} {{ $speaker->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($speaker->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($speaker->editorInfo!=null)
                                        {{ $speaker->editorInfo->name }} {{ $speaker->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($speaker->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش اسپیکر')
                                        <a href="{{ route('Speakers.edit',$speaker->id) }}">
                                            <button type="button" data-id="{{ $speaker->id }}"
