...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات نور</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد نور')
                    <a type="button" href="{{route('Lights.create')}}"
                ...
                @endcan
                @if(empty($lights) or $lights->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($lights as $light)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $light->adderInfo->name }} {{ $light->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($light->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($light->editorInfo!=null)
                                        {{ $light->editorInfo->name }} {{ $light->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($light->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش نور')
                                        <a href="{{ route('Lights.edit',$light->id) }}">
                                            <button type="button" data-id="{{ $light->id }}"
