...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">ایجاد قلم نوری</h1>
            @include('layouts.components.errors')
            <div class="bg-white rounded shadow flex flex-col ">
                {{ html()->form('POST')->route('DigitalPens.store')->acceptsFiles()->id('create-catalog')->open() }}
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                        ...
                        <div>
                            ...
                            <select name="connectivity_type"
                                ...
                                <option value="" disabled selected>انتخاب کنید</option>
                                    <option value="wired"
                                            @if(old('connectivity_type')=='wired') selected @endif>wired</option>
                                    <option value="wireless"
                                            @if(old('connectivity_type')=='wireless') selected @endif>wireless</option>
                                    <option value="hybrid"
                                            @if(old('connectivity_type')=='hybrid') selected @endif>hybrid</option>
                            </select>
                        </div>
                    </div>
                </div>
