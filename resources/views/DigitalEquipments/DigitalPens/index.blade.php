...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات قلم نوری</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد قلم نوری')
                    <a type="button" href="{{route('DigitalPens.create')}}"
                ...
                @endcan
                @if(empty($digitalPens) or $digitalPens->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($digitalPens as $digitalPen)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $digitalPen->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $digitalPen->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $digitalPen->connectivity_type }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($digitalPen->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $digitalPen->adderInfo->name }} {{ $digitalPen->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($digitalPen->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($digitalPen->editorInfo!=null)
                                        {{ $digitalPen->editorInfo->name }} {{ $digitalPen->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($digitalPen->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش قلم نوری')
                                        <a href="{{ route('DigitalPens.edit',$digitalPen->id) }}">
                                            <button type="button" data-id="{{ $digitalPen->id }}"
