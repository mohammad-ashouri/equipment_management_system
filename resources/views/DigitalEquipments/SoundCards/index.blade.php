...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات کارت صدا</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کارت صدا')
                    <a type="button" href="{{route('SoundCards.create')}}"
                ...
                @endcan
                @if(empty($soundCards) or $soundCards->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($soundCards as $soundCard)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $soundCard->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $soundCard->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $soundCard->connectivity_type }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($soundCard->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $soundCard->adderInfo->name }} {{ $soundCard->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($soundCard->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($soundCard->editorInfo!=null)
                                        {{ $soundCard->editorInfo->name }} {{ $soundCard->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($soundCard->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کارت صدا')
                                        <a href="{{ route('SoundCards.edit',$soundCard->id) }}">
                                            <button type="button" data-id="{{ $soundCard->id }}"
