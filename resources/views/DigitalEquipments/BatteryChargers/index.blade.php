...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات شارژر باتری</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد شارژر باتری')
                    <a type="button" href="{{route('BatteryChargers.create')}}"
                ...
                @endcan
                @if(empty($batteryChargers) or $batteryChargers->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($batteryChargers as $batteryCharger)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $batteryCharger->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $batteryCharger->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($batteryCharger->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $batteryCharger->adderInfo->name }} {{ $batteryCharger->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($batteryCharger->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($batteryCharger->editorInfo!=null)
                                        {{ $batteryCharger->editorInfo->name }} {{ $batteryCharger->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($batteryCharger->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش شارژر باتری')
                                        <a href="{{ route('BatteryChargers.edit',$batteryCharger->id) }}">
                                            <button type="button" data-id="{{ $batteryCharger->id }}"
