...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات کابینت باتری</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کابینت باتری')
                    <a type="button" href="{{route('BatteryCabinets.create')}}"
                ...
                @endcan
                @if(empty($batteryCabinets) or $batteryCabinets->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($batteryCabinets as $batteryCabinet)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $batteryCabinet->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $batteryCabinet->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($batteryCabinet->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $batteryCabinet->adderInfo->name }} {{ $batteryCabinet->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($batteryCabinet->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($batteryCabinet->editorInfo!=null)
                                        {{ $batteryCabinet->editorInfo->name }} {{ $batteryCabinet->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($batteryCabinet->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کابینت باتری')
                                        <a href="{{ route('BatteryCabinets.edit',$batteryCabinet->id) }}">
                                            <button type="button" data-id="{{ $batteryCabinet->id }}"
