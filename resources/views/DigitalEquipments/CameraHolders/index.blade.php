...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات پایه دوربین</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد پایه دوربین')
                    <a type="button" href="{{route('CameraHolders.create')}}"
                ...
                @endcan
                @if(empty($cameraHolders) or $cameraHolders->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">تعداد قطعات پایه</th>
                            <th class="px-6 py-3  font-bold ">نوع هد</th>
                        ...
                        @foreach ($cameraHolders as $cameraHolder)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $cameraHolder->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $cameraHolder->model }}
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    {{ $cameraHolder->parts_number }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $cameraHolder->head_type }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($cameraHolder->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $cameraHolder->adderInfo->name }} {{ $cameraHolder->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cameraHolder->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($cameraHolder->editorInfo!=null)
                                        {{ $cameraHolder->editorInfo->name }} {{ $cameraHolder->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cameraHolder->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش پایه دوربین')
                                        <a href="{{ route('CameraHolders.edit',$cameraHolder->id) }}">
                                            <button type="button" data-id="{{ $cameraHolder->id }}"
