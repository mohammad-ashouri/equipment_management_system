...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات پایه نور</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد پایه نور')
                    <a type="button" href="{{route('LightHolders.create')}}"
                ...
                @endcan
                @if(empty($lightHolders) or $lightHolders->isEmpty())
                ...
                @else
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($lightHolders as $lightHolder)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $lightHolder->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $lightHolder->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($lightHolder->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $lightHolder->adderInfo->name }} {{ $lightHolder->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($lightHolder->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($lightHolder->editorInfo!=null)
                                        {{ $lightHolder->editorInfo->name }} {{ $lightHolder->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($lightHolder->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش پایه نور')
                                        <a href="{{ route('LightHolders.edit',$lightHolder->id) }}">
                                            <button type="button" data-id="{{ $lightHolder->id }}"
