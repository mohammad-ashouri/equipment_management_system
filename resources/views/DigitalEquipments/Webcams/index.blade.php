...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات وبکم</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد وبکم')
                    <a type="button" href="{{route('Webcams.create')}}"
                ...
                @endcan
                @if(empty($webcams) or $webcams->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($webcams as $webcam)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $webcam->adderInfo->name }} {{ $webcam->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($webcam->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($webcam->editorInfo!=null)
                                        {{ $webcam->editorInfo->name }} {{ $webcam->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($webcam->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش وبکم')
                                        <a href="{{ route('Webcams.edit',$webcam->id) }}">
                                            <button type="button" data-id="{{ $webcam->id }}"
