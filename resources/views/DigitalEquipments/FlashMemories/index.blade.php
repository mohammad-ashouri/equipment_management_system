...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات فلش مموری</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد فلش مموری')
                    <a type="button" href="{{route('FlashMemories.create')}}"
                ...
                @endcan
                @if(empty($flashMemories) or $flashMemories->isEmpty())
                ...
                @else
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($flashMemories as $flashMemory)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $flashMemory->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $flashMemory->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $flashMemory->capacity }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($flashMemory->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $flashMemory->adderInfo->name }} {{ $flashMemory->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($flashMemory->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($flashMemory->editorInfo!=null)
                                        {{ $flashMemory->editorInfo->name }} {{ $flashMemory->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($flashMemory->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش فلش مموری')
                                        <a href="{{ route('FlashMemories.edit',$flashMemory->id) }}">
                                            <button type="button" data-id="{{ $flashMemory->id }}"
