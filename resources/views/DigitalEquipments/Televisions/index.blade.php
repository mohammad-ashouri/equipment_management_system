...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات تلوزیون</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد تلوزیون')
                    <a type="button" href="{{route('Televisions.create')}}"
                ...
                @endcan
                @if(empty($televisions) or $televisions->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($televisions as $television)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $television->adderInfo->name }} {{ $television->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($television->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($television->editorInfo!=null)
                                        {{ $television->editorInfo->name }} {{ $television->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($television->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش تلوزیون')
                                        <a href="{{ route('Televisions.edit',$television->id) }}">
                                            <button type="button" data-id="{{ $television->id }}"
