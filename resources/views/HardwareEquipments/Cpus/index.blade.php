...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات پردازنده</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد پردازنده')
                    <a type="button" href="{{route('Cpus.create')}}"
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($cpus as $cpu)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $cpu->adderInfo->name }} {{ $cpu->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cpu->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($cpu->editorInfo!=null)
                                        {{ $cpu->editorInfo->name }} {{ $cpu->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cpu->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش پردازنده')
                                        <a href="{{ route('Cpus.edit',$cpu->id) }}">
                                            <button type="button" data-id="{{ $cpu->id }}"
