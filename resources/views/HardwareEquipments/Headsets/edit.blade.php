...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">جزئیات و ویرایش هدست</h1>
            @include('layouts.components.errors')
            <div class="bg-white rounded shadow flex flex-col ">
                {{ html()->form('PATCH')->route('Headsets.update',$headset->id)->acceptsFiles()->id('edit-catalog')->open() }}
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                            ...
                            <select name="brand"
                                ...
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->id }}"
                                            @if($headset->brand==$brand->id) selected @endif>{{ $brand->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="text" name="model" value="{{ $headset->model }}"
                        ...
                        <div>
                            ...
                            <select name="connectivity_type"
                                ...
                                <option value="Bluetooth/Wlan"
                                        @if($headset->connectivity_type=='Bluetooth/Wlan') selected @endif>
                                    Bluetooth/Wlan
                                </option>
                        ...
                        <div>
                            ...
                            <select name="status"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="1" {{$headset->status ? 'selected' : ''}}>فعال</option>
                                <option value="0" {{!$headset->status ? 'selected' : ''}}>غیر فعال</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    @can('ویرایش هدست')
                        <input type="hidden" name="id" value="{{ $headset->id }}">
                        <button type="submit"
