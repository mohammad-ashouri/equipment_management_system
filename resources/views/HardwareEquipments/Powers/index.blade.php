...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات منبع تغذیه</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد منبع تغذیه')
                    <a type="button" href="{{route('Powers.create')}}"
                ...
                @endcan
                @if(empty($powers) or $powers->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($powers as $power)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $power->adderInfo->name }} {{ $power->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($power->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($power->editorInfo!=null)
                                        {{ $power->editorInfo->name }} {{ $power->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($power->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش منبع تغذیه')
                                        <a href="{{ route('Powers.edit',$power->id) }}">
                                            <button type="button" data-id="{{ $power->id }}"
