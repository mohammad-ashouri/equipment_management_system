...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات پرینتر</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد پرینتر')
                    <a type="button" href="{{route('Printers.create')}}"
                ...
                @endcan
                @if(empty($printers) or $printers->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($printers as $printer)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $printer->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $printer->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $printer->function_type }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    {{ $printer->adderInfo->name }} {{ $printer->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($printer->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($printer->editorInfo!=null)
                                        {{ $printer->editorInfo->name }} {{ $printer->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($printer->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش پرینتر')
