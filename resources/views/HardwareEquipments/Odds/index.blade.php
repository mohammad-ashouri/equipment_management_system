...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات درایو نوری</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد درایو نوری')
                    <a type="button" href="{{route('Odds.create')}}"
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($odds as $odd)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $odd->adderInfo->name }} {{ $odd->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($odd->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($odd->editorInfo!=null)
                                        {{ $odd->editorInfo->name }} {{ $odd->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($odd->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش درایو نوری')
                                        <a href="{{ route('Odds.edit',$odd->id) }}">
                                            <button type="button" data-id="{{ $odd->id }}"
