...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات مانیتور</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد مانیتور')
                    <a type="button" href="{{route('Monitors.create')}}"
                ...
                @endcan
                @if(empty($monitors) or $monitors->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($monitors as $monitor)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $monitor->adderInfo->name }} {{ $monitor->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($monitor->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($monitor->editorInfo!=null)
                                        {{ $monitor->editorInfo->name }} {{ $monitor->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($monitor->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش مانیتور')
                                        <a href="{{ route('Monitors.edit',$monitor->id) }}">
                                            <button type="button" data-id="{{ $monitor->id }}"
