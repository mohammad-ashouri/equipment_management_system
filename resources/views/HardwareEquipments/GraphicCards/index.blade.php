...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات کارت گرافیک</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کارت گرافیک')
                    <a type="button" href="{{route('GraphicCards.create')}}"
                ...
                @endcan
                @if(empty($graphicCards) or $graphicCards->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($graphicCards as $graphicCard)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $graphicCard->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $graphicCard->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $graphicCard->ram_size }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($graphicCard->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $graphicCard->adderInfo->name }} {{ $graphicCard->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($graphicCard->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($graphicCard->editorInfo!=null)
                                        {{ $graphicCard->editorInfo->name }} {{ $graphicCard->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($graphicCard->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کارت گرافیک')
                                        <a href="{{ route('GraphicCards.edit',$graphicCard->id) }}">
                                            <button type="button" data-id="{{ $graphicCard->id }}"
