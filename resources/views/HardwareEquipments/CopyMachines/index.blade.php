...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات دستگاه کپی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد دستگاه کپی')
                    <a type="button" href="{{route('CopyMachines.create')}}"
                ...
                @endcan
                @if(empty($copyMachines) or $copyMachines->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($copyMachines as $copyMachine)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $copyMachine->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $copyMachine->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($copyMachine->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $copyMachine->adderInfo->name }} {{ $copyMachine->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($copyMachine->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($copyMachine->editorInfo!=null)
                                        {{ $copyMachine->editorInfo->name }} {{ $copyMachine->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($copyMachine->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش دستگاه کپی')
                                        <a href="{{ route('CopyMachines.edit',$copyMachine->id) }}">
                                            <button type="button" data-id="{{ $copyMachine->id }}"
