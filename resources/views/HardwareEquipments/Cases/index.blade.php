...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات کیس</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد کیس')
                    <a type="button" href="{{route('Cases.create')}}"
                ...
                @endcan
                @if(empty($cases) or $cases->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($cases as $case)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $case->adderInfo->name }} {{ $case->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($case->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($case->editorInfo!=null)
                                        {{ $case->editorInfo->name }} {{ $case->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($case->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش کیس')
                                        <a href="{{ route('Cases.edit',$case->id) }}">
                                            <button type="button" data-id="{{ $case->id }}"
