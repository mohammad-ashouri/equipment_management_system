...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات سخت افزاری - مدیریت بر اطلاعات صفحه کلید</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد صفحه کلید')
                    <a type="button" href="{{route('Keyboards.create')}}"
                ...
                @endcan
                @if(empty($keyboards) or $keyboards->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($keyboards as $keyboard)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $keyboard->adderInfo->name }} {{ $keyboard->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($keyboard->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($keyboard->editorInfo!=null)
                                        {{ $keyboard->editorInfo->name }} {{ $keyboard->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($keyboard->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش صفحه کلید')
                                        <a href="{{ route('Keyboards.edit',$keyboard->id) }}">
                                            <button type="button" data-id="{{ $keyboard->id }}"
