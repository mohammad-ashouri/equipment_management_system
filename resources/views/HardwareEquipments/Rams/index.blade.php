    ...
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            ...
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد رم')
                    <a type="button" href="{{route('Rams.create')}}"
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($rams as $ram)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $ram->adderInfo->name }} {{ $ram->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($ram->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($ram->editorInfo!=null)
                                        {{ $ram->editorInfo->name }} {{ $ram->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($ram->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش رم')
                                        <a href="{{ route('Rams.edit',$ram->id) }}">
                                            <button type="button" data-id="{{ $ram->id }}"
