...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تعاریف اولیه - مدیریت بر اطلاعات قالب سوژه ها</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد قالب سوژه ها')
                    <a type="button" href="{{route('SubjectFormats.create')}}"
                ...
                @endcan
                @if(empty($subjectFormats) or $subjectFormats->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($subjectFormats as $subjectFormat)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $subjectFormat->adderInfo->name }} {{ $subjectFormat->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($subjectFormat->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($subjectFormat->editorInfo!=null)
                                        {{ $subjectFormat->editorInfo->name }} {{ $subjectFormat->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($subjectFormat->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش قالب سوژه ها')
                                        <a href="{{ route('SubjectFormats.edit',$subjectFormat->id) }}">
                                            <button type="button" data-id="{{ $subjectFormat->id }}"
