...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تعاریف اولیه - مدیریت بر اطلاعات مخاطب سوژه ها</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد مخاطب سوژه ها')
                    <a type="button" href="{{route('SubjectAudiences.create')}}"
                ...
                @endcan
                @if(empty($subjectAudiences) or $subjectAudiences->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($subjectAudiences as $subjectAudience)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $subjectAudience->adderInfo->name }} {{ $subjectAudience->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($subjectAudience->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($subjectAudience->editorInfo!=null)
                                        {{ $subjectAudience->editorInfo->name }} {{ $subjectAudience->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($subjectAudience->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش مخاطب سوژه ها')
                                        <a href="{{ route('SubjectAudiences.edit',$subjectAudience->id) }}">
                                            <button type="button" data-id="{{ $subjectAudience->id }}"
