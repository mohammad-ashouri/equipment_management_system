...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تعاریف اولیه - مدیریت بر اطلاعات شبکه اجتماعی</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد شبکه اجتماعی')
                    <a type="button" href="{{route('SocialMediaPlatforms.create')}}"
                ...
                @endcan
                @if(empty($socialMediaPlatforms) or $socialMediaPlatforms->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($socialMediaPlatforms as $socialMediaPlatform)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    @switch($socialMediaPlatform->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $socialMediaPlatform->adderInfo->name }} {{ $socialMediaPlatform->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($socialMediaPlatform->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($socialMediaPlatform->editorInfo!=null)
                                        {{ $socialMediaPlatform->editorInfo->name }} {{ $socialMediaPlatform->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($socialMediaPlatform->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش شبکه اجتماعی')
                                        <a href="{{ route('SocialMediaPlatforms.edit',$socialMediaPlatform->id) }}">
                                            <button type="button" data-id="{{ $socialMediaPlatform->id }}"
