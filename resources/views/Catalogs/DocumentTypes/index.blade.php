...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تعاریف اولیه - مدیریت بر اطلاعات انواع سند</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد نوع سند')
                    <a type="button" href="{{route('DocumentTypes.create')}}"
                ...
                @endcan
                @if(empty($documentTypes) or $documentTypes->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($documentTypes as $documentType)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $documentType->adderInfo->name }} {{ $documentType->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($documentType->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($documentType->editorInfo!=null)
                                        {{ $documentType->editorInfo->name }} {{ $documentType->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($documentType->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش نوع سند')
                                        <a href="{{ route('DocumentTypes.edit',$documentType->id) }}">
                                            <button type="button" data-id="{{ $documentType->id }}"
