<div class="bg-white mt-10 rounded shadow flex flex-col p-4">
    <h3 class="font-bold mb-5">اسناد پیوست (پس از وارد نمودن کد سند، کلید ctrl را فشار داده و منتظر پاسخ سرور
        باشید. در صورتی که سند شما یافت شود، نام سند در مقابل ورودی نمایش داده خواهد شد)</h3>
    <div class="grid gap-6 mb-6 md:grid-cols-1" id="documentsContainer">
        <div>
            @if(old('attached_documents'))
                @foreach(old('attached_documents') as $index => $attached_document)
                    <div class="flex">
                        <div class="">
                            ...
                            <input type="text" id="attached_documents{{ $index }}" name="attached_documents[]"
                                   value="{{ $attached_document }}" @keyup.ctrl="retrievePosts($event.target.value)"
    ...
    <div class="text-center">
        <button id="new_attached_document" type="button"
...
<script>
    document.getElementById('new_attached_document').addEventListener('click', function () {
