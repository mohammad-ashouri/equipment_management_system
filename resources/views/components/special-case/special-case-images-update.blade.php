                ...
                <div class="">
                    <div class=" mb-4">
                        <div class="w-full mr-3">
                            ...
                            <input type="text" id="saved_images_title" name="saved_images_title[]" value="{{$image->description}}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   placeholder="عنوان عکس را وارد کنید" required>
                        </div>
                        <div class="mr-3">
                            <label for="id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                عکس {{ $loop->iteration }}</label>
                            <img class="w-full h-96 cursor-pointer" title="برای بزرگنمایی کلیک کنید"
                                 @if($image!=null) onclick="openModal('{{ env('APP_URL') . $image->src }}')"
                                 @endif
                                 src="{{$image!=null ? env('APP_URL').$image->src : 'not found!'}}"
                                 alt="تصویر یافت نشد!">
                            <input type="hidden" value="{{ $image->src }}" name="saved_images[]">
                        </div>
                        <div class="mr-3 mt-7">
                            <button type="button"
