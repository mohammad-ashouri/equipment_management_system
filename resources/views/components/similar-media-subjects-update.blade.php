    ...
    <div class="grid gap-6 mb-6 md:grid-cols-1" id="mediaSubjectsContainer">
        <div>
            @if(!empty($mediaSubjects->similar_subjects))
                @foreach(json_decode($mediaSubjects->similar_subjects,true) as $index => $mediaSubject)
                ...
                console.error('Error fetching posts:', error);
                        ...
                        <div class="flex w-full">
                            ...
                            <input type="text" id="media_subjects{{ $index }}" name="media_subjects[]"
                                   ...
                                   required>
                            <span
                                class="mr-4 text-gray-900 dark:text-white">@php $subjectTitle=MediaSubject::where('id',$mediaSubject)->get()->pluck('title');  @endphp {{ $subjectTitle[0] }}</span>
                        </div>
                    </div>
                @endforeach
            @endif
