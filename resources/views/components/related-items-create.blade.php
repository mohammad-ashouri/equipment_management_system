<div class="bg-white mt-10 rounded shadow flex flex-col p-4">
    ...
    <div class="grid gap-6 mb-6 md:grid-cols-1" id="relatedItemsContainer">
        <div>
            @if(old('related_items'))
                @foreach(old('related_items') as $index => $relatedItem)
                    <div class="flex">
                        <div class="">
                            ...
                            <input type="text" id="related_items{{ $index }}" name="related_items[]"
                                   ...
                                   placeholder="عنوان مورد مرتبط {{ $index + 1 }} را وارد کنید"
                                   required>
                        </div>
                        <div class="mr-3">
                            ...
                            <select id="post_type{{ $index }}" name="post_types[]"
                                ...
                                @foreach($postTypes as $postType)
                                    <option
                                        value="{{ $postType->id }}" {{ old('post_types')[$index] == $postType->id ? 'selected' : '' }}>
                                        {{ $postType->title }}
                                    </option>
                                @endforeach
                            </select>
