...
<div class="bg-white mt-10 rounded shadow flex flex-col p-4">
    <h3 class="font-bold mb-5">یادداشت های مرتبط</h3>
                ...
                console.error('Error fetching posts:', error);
                        ...
                        <div class="flex w-full">
                            ...
                            <input type="text" id="notes{{ $index }}" name="notes[]"
                                   value="{{ $note->id }}" @keyup.ctrl="retrievePosts($event.target.value)"
                                   ...
                                   required>
                            <span
                                class="mr-4 text-gray-900 dark:text-white">@php $noteTitle=Note::where('id',$note->id)->get()->pluck('title');  @endphp {{ $noteTitle[0] }}</span>
                        </div>
                    </div>
                @endforeach
            @endif
