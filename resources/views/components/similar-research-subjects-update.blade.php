...
<div class="bg-white mt-10 rounded shadow flex flex-col p-4">
    ...
    <div class="grid gap-6 mb-6 md:grid-cols-1" id="researchSubjectsContainer">
        <div>
            @if(!empty($researchSubjects->similar_subjects))
                @foreach(json_decode($researchSubjects->similar_subjects,true) as $index => $researchSubject)
                ...
                console.error('Error fetching posts:', error);
                        ...
                        <div class="flex w-full">
                            ...
                            <input type="text" id="research_subjects{{ $index }}" name="research_subjects[]"
                                   ...
                                   required>
                            <span
                                class="mr-4 text-gray-900 dark:text-white">@php $subjectTitle=ResearchSubject::where('id',$researchSubject)->get()->pluck('title');  @endphp {{ $subjectTitle[0] }}</span>
                        </div>
                    </div>
                @endforeach
            @endif
