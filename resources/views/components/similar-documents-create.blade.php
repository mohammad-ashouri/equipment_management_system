<div class="bg-white mt-10 rounded shadow flex flex-col p-4">
    <h3 class="font-bold mb-5">اسناد مرتبط (پس از وارد نمودن کد سند، کلید ctrl را فشار داده و منتظر پاسخ سرور
    ...
    <div class="grid gap-6 mb-6 md:grid-cols-1" id="documentsContainer">
        <div>
            @if(old('similar_documents'))
                @foreach(old('similar_documents') as $index => $similar_document)
                    <div class="flex">
                        <div class="">
                            ...
                            <input type="text" id="similar_documents{{ $index }}" name="similar_documents[]"
                                   value="{{ $similar_document }}" @keyup.ctrl="retrievePosts($event.target.value)"
