<div class="bg-white mt-10 rounded shadow flex flex-col p-4">
    <h3 class="font-bold mb-5">فیلم کوتاه های مرتبط (پس از وارد نمودن کد فیلم کوتاه، کلید ctrl را فشار داده و منتظر پاسخ
        سرور
        باشید. در صورتی که فیلم کوتاه شما یافت شود، نام فیلم کوتاه در مقابل ورودی نمایش داده خواهد شد)</h3>
    <div class="grid gap-6 mb-6 md:grid-cols-1" id="shortVideosContainer">
        <div>
            @if(old('short_videos'))
                @foreach(old('short_videos') as $index => $shortVideo)
                    <div class="flex">
                        <div class="">
                            ...
                            <input type="text" id="short_videos{{ $index }}" name="short_videos[]"
                                   value="{{ $shortVideo }}" @keyup.ctrl="retrievePosts($event.target.value)"
...
`;
        document.getElementById('shortVideosContainer').insertAdjacentHTML('beforeend', newInput);
