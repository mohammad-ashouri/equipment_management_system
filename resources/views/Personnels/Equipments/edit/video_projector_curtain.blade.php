    ...
    <select name="video_projector_curtain"
        ...
        @foreach($videoProjectorCurtains as $videoProjectorCurtain)
            <option value="{{ $videoProjectorCurtain->id }}"
                    @if($equipmentInfo['video_projector_curtain']==$videoProjectorCurtain->id) selected @endif>{{ $videoProjectorCurtain->brandInfo->name}}
                - {{ $videoProjectorCurtain->model}} - ارتفاع: {{ $videoProjectorCurtain->height}} - عرض: {{ $videoProjectorCurtain->width}}</option>
        @endforeach
    </select>
</div>
