...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات شبکه - مدیریت بر اطلاعات تستر شبکه</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد تستر شبکه')
                    <a type="button" href="{{route('CableTesters.create')}}"
                ...
                @endcan
                @if(empty($cableTesters) or $cableTesters->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($cableTesters as $cableTester)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $cableTester->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $cableTester->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($cableTester->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $cableTester->adderInfo->name }} {{ $cableTester->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cableTester->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($cableTester->editorInfo!=null)
                                        {{ $cableTester->editorInfo->name }} {{ $cableTester->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($cableTester->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش تستر شبکه')
                                        <a href="{{ route('CableTesters.edit',$cableTester->id) }}">
                                            <button type="button" data-id="{{ $cableTester->id }}"
