...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تعاریف اولیه - مدیریت بر اطلاعات رک</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد رک')
                    <a type="button" href="{{route('Racks.create')}}"
                ...
                @endcan
                @if(empty($racks) or $racks->isEmpty())
                ...
                @else
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        <tr class="bg-gradient-to-r from-blue-400 to-purple-500 items-center text-center text-white">
                            ...
                            <th class="px-6 py-3  font-bold ">تعداد یونیت</th>
                        ...
                        @foreach ($racks as $rack)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $rack->adderInfo->name }} {{ $rack->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($rack->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($rack->editorInfo!=null)
                                        {{ $rack->editorInfo->name }} {{ $rack->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($rack->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش رک')
                                        <a href="{{ route('Racks.edit',$rack->id) }}">
                                            <button type="button" data-id="{{ $rack->id }}"
