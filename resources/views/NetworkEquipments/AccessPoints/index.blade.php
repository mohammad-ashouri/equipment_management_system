...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات شبکه - مدیریت بر اطلاعات اکسس پوینت</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد اکسس پوینت')
                    <a type="button" href="{{route('AccessPoints.create')}}"
                ...
                @endcan
                @if(empty($accessPoints) or $accessPoints->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($accessPoints as $accessPoint)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $accessPoint->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $accessPoint->model }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $accessPoint->antennas_number }}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    {{ $accessPoint->adderInfo->name }} {{ $accessPoint->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($accessPoint->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($accessPoint->editorInfo!=null)
                                        {{ $accessPoint->editorInfo->name }} {{ $accessPoint->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($accessPoint->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش اکسس پوینت')
                                        <a href="{{ route('AccessPoints.edit',$accessPoint->id) }}">
                                            <button type="button" data-id="{{ $accessPoint->id }}"
