...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">سایر تجهیزات دیجیتال - مدیریت بر اطلاعات nvr</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد nvr')
                    <a type="button" href="{{route('Nvrs.create')}}"
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($nvrs as $nvr)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $nvr->adderInfo->name }} {{ $nvr->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($nvr->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($nvr->editorInfo!=null)
                                        {{ $nvr->editorInfo->name }} {{ $nvr->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($nvr->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش nvr')
                                        <a href="{{ route('Nvrs.edit',$nvr->id) }}">
                                            <button type="button" data-id="{{ $nvr->id }}"
