    ...
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            ...
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد مودم')
                    <a type="button" href="{{route('Servers.create')}}"
                ...
                @endcan
                @if(empty($servers) or $servers->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($servers as $server)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $server->adderInfo->name }} {{ $server->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($server->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($server->editorInfo!=null)
                                        {{ $server->editorInfo->name }} {{ $server->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($server->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش مودم')
                                        <a href="{{ route('Servers.edit',$server->id) }}">
                                            <button type="button" data-id="{{ $server->id }}"
