...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات شبکه - مدیریت بر اطلاعات روتر</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد روتر')
                    <a type="button" href="{{route('Routers.create')}}"
                ...
                @endcan
                @if(empty($routers) or $routers->isEmpty())
                    ...
                    <table class="datatable w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($routers as $router)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $router->adderInfo->name }} {{ $router->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($router->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($router->editorInfo!=null)
                                        {{ $router->editorInfo->name }} {{ $router->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($router->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش روتر')
                                        <a href="{{ route('Routers.edit',$router->id) }}">
                                            <button type="button" data-id="{{ $router->id }}"
