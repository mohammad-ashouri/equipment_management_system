...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تعاریف اولیه - مدیریت بر اطلاعات سوییچ</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد سوییچ')
                    <a type="button" href="{{route('Switches.create')}}"
                ...
                @endcan
                @if(empty($switches) or $switches->isEmpty())
                    ...
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($switches as $switch)
                            <tr class="bg-white">
                                ...
                                <td class="px-6 py-4">
                                    {{ $switch->adderInfo->name }} {{ $switch->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($switch->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($switch->editorInfo!=null)
                                        {{ $switch->editorInfo->name }} {{ $switch->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($switch->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش سوییچ')
                                        <a href="{{ route('Switches.edit',$switch->id) }}">
                                            <button type="button" data-id="{{ $switch->id }}"
