...
@section('content')
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            <h1 class="text-2xl font-bold mb-4">تجهیزات شبکه - مدیریت بر اطلاعات رادیو وایرلس</h1>
            @include('layouts.components.errors')
            @include('layouts.components.success')
            <div class="bg-white rounded shadow p-6 flex flex-col ">
                @can('ایجاد رادیو وایرلس')
                    <a type="button" href="{{route('RadioWirelesses.create')}}"
                ...
                @endcan
                @if(empty($radioWirelesses) or $radioWirelesses->isEmpty())
                ...
                @else
                    <table class="w-full border-collapse rounded-lg overflow-hidden text-center datasheet">
                        ...
                        @foreach ($radioWirelesses as $radioWireless)
                            <tr class="bg-white">
                                <td class="px-6 py-4">{{ $loop->iteration }}</td>
                                <td class="px-6 py-4">
                                    {{ $radioWireless->brandInfo->name }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ $radioWireless->model }}
                                </td>
                                <td class="px-6 py-4">
                                    @switch($radioWireless->status)
                                        @case(1)
                                            فعال
                                            @break
                                        @case(0)
                                ...
                                <td class="px-6 py-4">
                                    {{ $radioWireless->adderInfo->name }} {{ $radioWireless->adderInfo->family }}
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($radioWireless->created_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @if($radioWireless->editorInfo!=null)
                                        {{ $radioWireless->editorInfo->name }} {{ $radioWireless->editorInfo->family }}
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    {{ Jalalian::fromDateTime($radioWireless->updated_at)->format('H:i:s Y/m/d') }}
                                </td>
                                <td class="px-6 py-4">
                                    @can('ویرایش رادیو وایرلس')
                                        <a href="{{ route('RadioWirelesses.edit',$radioWireless->id) }}">
                                            <button type="button" data-id="{{ $radioWireless->id }}"
