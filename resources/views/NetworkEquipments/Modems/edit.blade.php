    ...
    <main class="flex-1 bg-gray-100 py-6 px-8">
        <div class="mx-auto lg:mr-72">
            ...
            <div class="bg-white rounded shadow flex flex-col ">
                {{ html()->form('PATCH')->route('Modems.update',$modem->id)->acceptsFiles()->id('edit-catalog')->open() }}
                    ...
                    <div class="grid gap-6 mb-6 md:grid-cols-2">
                            ...
                            <select name="brand"
                                ...
                                @foreach($brands as $brand)
                                    <option value="{{ $brand->id }}"
                                            @if($modem->brand==$brand->id) selected @endif>{{ $brand->name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="text" name="model" value="{{ $modem->model }}"
                        ...
                        <div>
                            ...
                            <select name="connectivity_type"
                                    ...
                                    required>
                                <option value="" disabled selected>انتخاب کنید</option>
                                <option value="wired" @if($modem->connectivity_type=='wired') selected @endif>
                                    wired
                                </option>
                        ...
                        <div>
                            <label for="ports_number"
                                ...
                                پورت </label>
                            <input type="number" name="ports_number" value="{{ $modem->ports_number }}"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                   placeholder="" required>
                        </div>
                        <div>
                            ...
                            <select name="type"
                                    ...
                                    required>
                                <option value="" disabled selected>انتخاب کنید</option>
                                <option value="ADSL2+" @if($modem->type=='ADSL2+') selected @endif>
                                    ADSL2+
                                </option>
                        ...
                        <div>
                            ...
                            <select name="antennas_number"
                                ...
                                @for($i=1;$i<5;$i++)
                                    <option value="{{$i}}"
                                            @if($modem->antennas_number==$i) selected @endif>{{$i}}</option>
                                @endfor
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="status"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                    required>
                                <option value="1" {{$modem->status ? 'selected' : ''}}>فعال</option>
                                <option value="0" {{!$modem->status ? 'selected' : ''}}>غیر فعال</option>
                            </select>
                        </div>
                    </div>
                </div>
