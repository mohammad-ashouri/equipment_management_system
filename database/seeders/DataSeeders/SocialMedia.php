...
//                        'img_src' => $img_src
    ...
    public function run(): void
    {
        ...
        $of_posts = DB::connection('mysql2')->table('of_posts')->where('post_type', 'cyber')->where(function ($query) {
        ...
        foreach ($of_posts as $of_post) {
            ...
            foreach ($postAttachments as $postAttachment) {
                ...
                if ($src != '-' and !empty($src)) {
                    ...
                    $picture->post_type = "social_media";
                    if (!$isChild) {
            ...
            $platformId = $socialID = $followers = $activity = null;
            foreach ($of_postmeta as $of_postmeta_item) {
                switch ($of_postmeta_item->meta_key) {
                    case 'ID':
                        $socialID = $of_postmeta_item->meta_value;
                        if (empty($socialID)) {
                            $socialID = null;
                        }
                        break;
                    case 'social_media':
                        $platformId = SocialMediaPlatform::firstOrCreate(['name' => $of_postmeta_item->meta_value,'icon_src'=>'/build/images/'.$of_postmeta_item->meta_value.'.png', 'adder' => 10]);
                        break;
                    case 'follower':
                        $followers = $of_postmeta_item->meta_value;
                        if (empty($followers)) {
                            $followers = null;
                        }
                        break;
                    case 'Activity':
                        $activity = $of_postmeta_item->meta_value;
                        if (empty($activity)) {
                            $activity = null;
                        }
                        break;
            ...
            $socialMedia = new \App\Models\SocialMedia();
            $socialMedia->id = $of_post->ID;
            $socialMedia->title = $of_post->post_title;
            $socialMedia->body = $cleanedHtml;

            $socialMedia->platform = $platformId->id;
            $socialMedia->platform_id = $socialID;
            ...
            $socialMedia->adder = $of_post->post_author;
            $socialMedia->created_at = $of_post->post_date;
            $socialMedia->updated_at = $of_post->post_modified;
            $socialMedia->save();
        }
    }
}
