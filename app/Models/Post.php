...
class Post extends Model
{
    ...
    {
        return $this->belongsTo(Picture::class, 'id', 'post_id')->where('post_type', 'post')->where('image_type', 'picture_main');
    }

    public function childImages()
    {
        return $this->hasMany(Picture::class, 'post_id')->where('post_type', 'post')->where('image_type', 'picture_child');
    }
    ...
    {
        return $this->belongsToMany(Category::class, 'category_relationships', 'post_id', 'category_id')
            ->withTimestamps()
            ->withPivot('type')
            ->where('type', '=', 'post'); // You can include pivot columns if needed
    }

    public function sliderImage()
    {
        return $this->belongsTo(Picture::class, 'id', 'post_id')->where('post_type', 'post')->where('image_type', 'slider');
    }
}
